...
- Why is `express.Router()` used in Express.js applications, and how does it benefit the code structure?
   ...
   `express.Router()` is used within Express.js to help group related routes and middleware together, because it allows the creation
   of modular and mountable route handlers it makes our code more organized and easier to maintain in the future. 
...
- How would you implement error handling in the Express routes to ensure that any issues (such as file not found or server errors) are appropriately handled? Provide an example.
   ...
   You should implement error handdling in the Express routes by using `if` statements and `try/catch` blocks  to
   deal errors found and then use error handling middleware by calling `next(err)` and then addressing the error.
...
- Explain how the `app.listen(process.env.port || 8081)` line works and why it's useful in production environments.
   ...
   The line above works by listening to your environment port 8081 and is useful because in producation environments it avoids conflicts with system-level environment variables like `PATH` or `USER`. It seperates application level configs compared to global environment settings. But if we were deploying to a platform using uppercase `PORT` would be more usefull because they usually inject a environment variable into `PORT` and if it is lowercase there is chance it won't find it which means request never reach your app
